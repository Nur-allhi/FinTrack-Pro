...
router.get("/", async (req, res) => {
  try {
    ...
    logger.error({ requestId: req.requestId, error: message }, "GET /api/investments");
...
router.get("/:id/returns", async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Unknown error';
    logger.error({ requestId: req.requestId, error: message }, "GET /api/investments/:id/returns");
    sendError(res, 500, message, "INTERNAL_ERROR");
  }
});
...
router.post("/", async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Unknown error';
    logger.error({ requestId: req.requestId, error: message }, "POST /api/investments");
    sendError(res, 500, message, "INTERNAL_ERROR");
  }
});
...
router.post("/:id/returns", async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Unknown error';
    logger.error({ requestId: req.requestId, error: message }, "POST /api/investments/:id/returns");
    sendError(res, 500, message, "INTERNAL_ERROR");
  }
});
