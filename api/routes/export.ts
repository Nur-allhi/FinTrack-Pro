...
router.get("/", async (req, res) => {
  try {
    ...
    logger.error({ requestId: req.requestId, error: message }, "GET /api/export");
...
router.post("/", async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Unknown error';
    logger.error({ requestId: req.requestId, error: message }, "POST /api/import");
    sendError(res, 500, message, "INTERNAL_ERROR");
  }
});
...
router.delete("/clear-all", async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Unknown error';
    logger.error({ requestId: req.requestId, error: message }, "DELETE /api/export/clear-all");
    sendError(res, 500, message, "INTERNAL_ERROR");
  }
});
