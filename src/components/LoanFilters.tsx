      ...
      <div className="flex items-center gap-1 p-0.5 bg-surface-soft rounded-pill relative shadow-sm">
        {(['all', 'active', 'settled'] as const).map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            className={cn(
              "relative px-3 py-1 rounded-pill text-xs font-bold uppercase tracking-wider transition-colors flex-1 text-center",
      ...
      <div className="flex items-center gap-1 p-0.5 bg-surface-soft rounded-pill relative shadow-sm">
        {(['pair', 'borrower'] as const).map(g => (
          <button key={g} onClick={() => setGroupingMode(g)}
            className={cn(
              "relative flex items-center justify-center gap-1.5 px-3 py-1 rounded-pill text-xs font-bold transition-colors flex-1",
              groupingMode === g ? "text-ink" : "text-muted hover:text-ink"
            )}>
            {g === 'pair' ? <ArrowLeftRight className="w-3 h-3" /> : <Users className="w-3 h-3" />}
            {g === 'pair' ? 'By Pair' : 'By Borrower'}
            {groupingMode === g && (
